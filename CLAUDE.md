...
js/indicators.js      12 наборов индикаторов (6..100 px), вшиты из src_doc/indicators
...
| Размер индикаторов | Подпись ползунка — «Индикаторы Ø45». По умолчанию 45 px, обводка 3.4 — как во всех файлах `src_doc/examples`. КОМПАКС не читает `scale`, поэтому размеры не масштабируются, а берутся готовыми наборами путей: 6/0.5, 10/0.8, 16/1.2, 20/1.6, 24/2, 28/2.1, 32/2.4, 38/2.9, 45/3.4, 60/4.6, 82/6, 100/8 в `js/indicators.js`. Диаметр — внешний, обводка в него входит. Ползунок берёт список из ключей таблицы, и концы своего диапазона тоже: чтобы добавить размер, достаточно дописать набор. Число в подписи держит ширину самого широкого из размеров, иначе панель дёргалась бы при каждом шаге ползунка. Ширина **измеряется** (`reserveSizeWidth`, пробный элемент за краем страницы, потому что панель до загрузки файла скрыта и меряться не может), а не пишется в css: `ch` — это ширина нуля без табличных цифр, и «100» в три таких не влезает |
...
area back, and
Ctrl+Z for putting back the last change and nothing before it.
...
2. **Наборы индикаторов — настоящие, но их исходников в репозитории не будет.**
   Двенадцать наборов в `js/indicators.js` (6…100 px) собраны из
   `src_doc/indicators` — по файлу на размер, `ind_<диаметр>_<обводка>.svg`,
   диаметр внешний, вместе с обводкой. Папку обещали удалить, в репозиторий она
   не попала, поэтому вся геометрия вшита в модуль и приложение на неё не
   ссылается. Новый размер
   добавляется своим набором (`strokeWidth` + `icons`), формат таблицы менять
   не нужно: из ключей берутся и список ползунка, и концы его диапазона. Семь
   прежних наборов совпали с новыми файлами байт в байт, поэтому эталоны
